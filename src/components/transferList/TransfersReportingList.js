...
export const TransfersReportingList = (props) => {
      ...
      <Card border="light" className="table-wrapper table-responsive shadow-sm">
        <Card.Body className="pt-0">
          <Table hover className="user-table align-items-center">
            ...
            <tbody>
              {listInfo.map((t) => (
                <TransfersReportingList.TableRow key={`transaction-${t.id}`} {...t} />
              ))}
            </tbody>
          </Table>
...
TransfersReportingList.TableRow = (props) => {
  let {
    transactionId,
    type,
    merchantId,
  ...
  return (
    <tr>
      ...
      <td>
        <span className="fw-normal text-wrap">{createdAt}</span>
      </td>
      <td>
        <span className="fw-normal text-wrap">{updatedAt}</span>
      </td>
    </tr>
  );
};
