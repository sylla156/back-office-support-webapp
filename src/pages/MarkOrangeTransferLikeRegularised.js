...
import {
  ...
  FIRST_PAGE_INDEX,
  EXPORT_ORANGE_REPORT_TRANSFER_MARK_LIKE_REGULARISED
} from "./constante/Const";
import { Redirect } from "react-router-dom";
import { Routes } from "../routes";
import { format, addMinutes, parseISO, subDays } from "date-fns";
import AlertDismissable from "../components/AlertDismissable";
import { OrangeReportTransferMarkLikeRegularisedImportFile } from "./OrangeReportTransferMarkLikeRegularisedImportFile";
import { MarkOrangeTransferLikeRegularisedList } from "./MarkOrangeTransferLikeRegularisedList";
...
export default()=>{
  ...
  const getMarkOrangeReportTransferListRegularised = ()=> {
    setIsLoaded(false);
    setErrorData(null);
    axios
    .get(GET_MARK_ORANGE_REPORT_TRANSFER_LIKE_REGULARISED, {
      params: {
        from: startDate,
        to: endDate,
        page: currentPage,
  ...
  const exportData = () => {
    ...
    axios
      .get(EXPORT_ORANGE_REPORT_TRANSFER_MARK_LIKE_REGULARISED, {
        params: {
          from: startDate,
          to: endDate,
          csv: true,
  ...
  useEffect(() => {
    getMarkOrangeReportTransferListRegularised();
  }, [currentPage, version]);
  ...
  return (
    <>
        {userCanAddOrangeTransactiontRegularised && <OrangeReportTransferMarkLikeRegularisedImportFile
        ...
        <Col xs={12} md={3} lg={3} className="px-2 mt-4">
          <div className="mt-3 mb-4">
            ...
            <Button
              ...
              variant="primary"
              type="button"
              onClick={getMarkOrangeReportTransferListRegularised}
            >
              Filtrer
            </Button>
      ...
      {isLoaded ? (
        <Row>
          <MarkOrangeTransferLikeRegularisedList
            key={version}
