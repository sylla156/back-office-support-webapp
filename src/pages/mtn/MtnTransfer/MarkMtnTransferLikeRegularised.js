...
import { APPKEY, FIRST_PAGE_INDEX, GET_MARK_MTN_REPORT_TRANSFER_LIKE_REGULARISED, EXPORT_MTN_REPORT_TRANSFER_MARK_LIKE_REGULARISED } from '../../constante/Const';
import { Redirect } from 'react-router-dom';
import { Routes } from '../../../routes';
import { format, addMinutes, subDays } from 'date-fns';
import AlertDismissable from '../../../components/AlertDismissable';
import { MarkMtnTransferLikeRegularisedList } from './MarkMtnTransferLikeRegularisedList';
...
export default () => {
    ...
    const userCanAddMtnTransferRegularised = cookies.user.canAddPaymentRegularised;

    const axios = AxiosWebHelper.getAxios();

    const getMarkMtnReportTransferListRegularised = () => {
        setIsLoaded(false)
        setErrorData(null)
        axios.get(GET_MARK_MTN_REPORT_TRANSFER_LIKE_REGULARISED, {
            params: {
                from: startDate,
                to: endDate,
            },
    ...
    const fileName = "mtn-report-transfer-who-must-be-regularise-export";
    const exportData = () => {
        setErrorDataCSV(null);
        setIsLoadedCSV(false);

        axios.get(EXPORT_MTN_REPORT_TRANSFER_MARK_LIKE_REGULARISED, {
            params: {
                from: startDate,
                to: endDate,
            },
    ...
    useEffect(() => {
        getMarkMtnReportTransferListRegularised();
    }, [currentPage, version]);
    ...
    return (
        <>
                ...
                <Col xs={12} md={3} lg={3} className="px-2 mt-4">
                    <div className="mt-3 mb-4">
                        ...
                        <Button
                            ...
                            variant="primary"
                            type="button"
                            onClick={() => getMarkMtnReportTransferListRegularised()}
                        >
                            Filtrer
                        </Button>
            ...
            {isLoaded ? (
                <Row>
                    <MarkMtnTransferLikeRegularisedList
                        key={version}
